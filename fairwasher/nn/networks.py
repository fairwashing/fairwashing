...
from fairwasher.nn.layers import Convolutional, Dense, MaxPool

from fairwasher.nn.enums import LRPRule, ExplainingMethod


class ExplainableNet(nn.Module):
    def __init__(self, model=None, data_mean=np.array([0.485, 0.456, 0.406]), data_std=np.array([0.229, 0.224, 0.225]),
                 lrp_rule_first_layer=LRPRule.z_b, lrp_rule_next_layers=LRPRule.z_plus, beta=None):
    ...
    def change_beta(self, beta):
        ...
        for layer in self.layers:
            if hasattr(layer, "activation_fn") and layer.activation_fn is not None:
                if self.beta_activation is None:
                    layer.activation_fn = torch.nn.ReLU()
                else:
                    layer.activation_fn = torch.nn.Softplus(beta=self.beta_activation)
    ...
    def analyze(self, method=ExplainingMethod.lrp, R=None, index=None):
        ...
        if index is not None:
            R = self.R.clone()
            indices = torch.zeros(10).byte()
            indices[index] = 1
            R[0] = R[0] * indices.float().cuda()
