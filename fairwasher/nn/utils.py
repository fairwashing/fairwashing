...
from torchvision.transforms import Normalize, ToTensor, CenterCrop, Resize
...
def plot_overview(images, heatmaps, mean, std,
                  captions=['target img', 'org img', 'adv img', 'target heatmap', 'org heatmap', 'adv heatmap'],
    ...
    plot_grid(plots, captions, cmap=cmaps, filename=filename)
...
def load_image(data_mean, data_std, device, image_name):
    ...
    x = Normalize(mean=data_mean, std=data_std)(ToTensor()(CenterCrop(224)(Resize(256)(im))))
...
def get_heatmap(model, x, method, relevance_per_pixel=True, desired_index=None):
    """
