from fairwasher.nn.enums import LRPRule
from fairwasher.nn.utils import CovarianceCalculator
        ...
        if self.activation_fn is not None:
            if hasattr(self.activation_fn, "beta"):
                R = torch.nn.functional.softplus(
                    R,
                    beta=self.activation_fn.beta
                ) * torch.sigmoid(self.activation_fn.beta * self.pre_activation)
            else:
